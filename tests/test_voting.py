...
def test_voting(votingContract, accounts):
    sampleContract = votingContract.address
    
    #testing propose vote
    ...
    assert votingContract.endBlock(accounts[2]) == 0, "Non-existent Vote should not have data"
    assert votingContract.storedDonation(accounts[2]) == 0, "Empty votes should not have money in them"
    
    #testing vote
    votingContract.mint(accounts[1], 10, {'from': accounts[0]}) # adds 1000VC to accounts balance
    ...
    assert votingContract.totalSupply() == 10010
    ...
    assert votingContract.voterCoinStaked() == 10

    votingContract.proposeVote(secondWinningProp, "you should also vote for this thing", {'from': accounts[1], 'value': 1000})
